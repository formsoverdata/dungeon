...
unsigned char player_direction; // NORTH,EAST,SOUTH,WEST
...
unsigned char get_tile(unsigned char x, unsigned char y, unsigned char px, unsigned char py)
{
    unsigned char mx;
    unsigned char my;
    ...
    if (mx == px && my == py)
        return get_map_tile(px, py); // useful to know we can trap this

    if (mx < MAP_HEIGHT && my < MAP_WIDTH)
        return get_map_tile(mx, my);

    return 0;
}

void draw_row(signed char x, signed char x2, unsigned char px, unsigned char py)
{
    ...
    for (unsigned char ty = 0; ty <= 15; ty++)
    {
        if (ty >= MAP_Y_START && ty <= MAP_Y_END)
        {
            ...
            unsigned char tile = get_tile(x, y, px, py);
            unsigned char tile2 = x == x2 ? tile : get_tile(x2, y, px, py);
            tile = tile >> 3 | tile2;
            *attr_address++ = tile;
            *attr_address++ = tile;
        }
...
void draw_map(unsigned char frame, unsigned char sub_frame, unsigned char px, unsigned char py)
{    
    ...
    {
        draw_row(x - sub_frame, x, px, py);
        draw_row(x, x, px, py);
        x++;
    }

    fill_rectangle_attr(16, 16, 2, 2, 7, 7); // player square
    fill_rectangle_attr(23, 6, 1, 20, 2, 7); // hide bottom row
    copy_attr_buffer();
}

static inline void animate_forward(unsigned char prev_x, unsigned char prev_y)
{   
    draw_map(0, 1, prev_x, prev_y); 
    draw_map(1, 0, prev_x, prev_y);
    draw_map(1, 1, prev_x, prev_y);
    draw_map(0, 0, player_x, player_y); // final position
}

static inline void animate_backward()
{   
    draw_map(1, 1, player_x, player_y);
    draw_map(1, 0, player_x, player_y);
    draw_map(0, 1, player_x, player_y); 
    draw_map(0, 0, player_x, player_y); // final position
}
...
void loop_forwards()
{
    ...
    draw_map(0, 0, player_x, player_y);

    // forward
    player_direction = NORTH;
    for (player_x = MAP_HEIGHT - 1; player_x > 0;)
    {
        move_forward();
        move_backward();
        move_forward();
    ...
    for (player_y = 0; player_y < MAP_WIDTH - 1;)
    {
        move_forward();
        move_backward();
        move_forward();
    ...
    for (player_x = 0; player_x < MAP_HEIGHT - 1;)
    {
        move_forward();
        move_backward();
        move_forward();
    ...
    for (player_y = MAP_WIDTH - 1; player_y > 0;)
    {
        move_forward();
        move_backward();
        move_forward();
...
{       
    ...
    do
    {
        ...
        copy_attr_buffer();
        fill_rectangle_char(0, 0, 24, 32, "["); // horizontal stripe
        fill_rectangle_attr(0, 0, 24, 32, 2, 7);
        copy_attr_buffer();
        // loop around map
        loop_forwards();
    } while (1);
